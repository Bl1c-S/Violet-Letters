...
import { requestAdapterWithRefreshStage } from "@/app/components/reqest/reqestAdapter";
...
const LettersList = () => {
  ...
  useEffect(() => {
    const fetchLetters = async () => {
      ...
      try {
        const fetchedLetters = await getAllLetters(user.id);
        ...
        >
          ...
          <div className="flex flex-row flex-w justify-center gap-2">
            <Link href={`/letter/${letter.id}`}>
              <button className="w-full py-2 px-4 bg-[#c5a193] hover:bg-[#ffddb4] text-white font-semibold rounded-lg shadow-md focus:ring-[#cb3950] focus:ring-offset-2 focus:outline-none focus:ring-2">
                Изменить
              </button>
            </Link>
...
const getAllLetters = async (user_id: string): Promise<Letter[]> => {
